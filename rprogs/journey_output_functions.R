...
batch_processing = function(df, event_identity, sequence_name,
  ...
  assert(length(list_of_identities) / number_of_batches < 10000, "too few batches given size of dataframe")
...
distance_function = function(single_timeline, group_timelines){
  ...
  assert(all(single_timeline %in% c(0,1)), "timelines must be binary")
  assert(all(sapply(group_timelines, function(x) all(x %in% c(0,1)))), "timelines must be binary")
