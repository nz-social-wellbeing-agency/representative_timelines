...
produce_event_stages = function(db_connection, schema, output_table_name, event_w_roles, event_w_date, controls){
  ...
  for(row_num in 1:nrow(controls)){
    ...
    append_database_table(db_connection, schema, "jt_tmp_stage_events", 
                          names(event_stage_table_columns), event_stage)
  }
    
  ## output ----
  
  result = create_clustered_index(db_connection, schema, "jt_tmp_stage_events", "snz_uid")
  
  required_columns = c("snz_uid", "role", "event_id", "start_date", "end_date", "stage_name", "time_unit", "time")
  event_stages = create_access_point(db_connection, schema, tbl_name = "jt_tmp_stage_events") %>%
...
compact_spells = function(tbl_name, within, start_date_col, end_date_col, overlap_days,
  ...
  marked_for_merge = write_for_reuse(db_connection, schema, "jt_tmp_marked_for_merge", marked_for_merge)
...
combine_spells = function(tbl_name_1, tbl_name_2, within, start_date_col, end_date_col,
  ...
  core_list_of_dates = write_for_reuse(db_connection, schema, "jt_tmp_core_list_of_dates",
  ...
  start_date_1 = paste0(start_date_col,"_1")
  start_date_2 = paste0(start_date_col,"_2")
  end_date_1 = paste0(end_date_col,"_1")
  end_date_2 = paste0(end_date_col,"_2")
  
  core_1 = core_list_of_dates %>%
    inner_join(tbl_name_1, by = within, suffix = c("","_1")) %>%
    filter(!!sym(start_date_col) <= !!sym(end_date_1)
           & !!sym(start_date_1) <= !!sym(end_date_col))
  
  core_2 = core_list_of_dates %>%
    inner_join(tbl_name_2, by = within, suffix = c("","_2")) %>%
